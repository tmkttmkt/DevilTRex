...
public class story : MonoBehaviour
{
    ...
    public void reba_ugo_flg()
    {
        if (!reba_uku_flg) return;
        int i = 0;
        reba[] instances = FindObjectsOfType<reba>();

        // 各インスタンスに対してB変数の値を表示
        foreach (reba instance in instances)
        {
            if (instance.flg)
            {
                i++;
        ...
        ti.set_text("あと残り" + (4-i).ToString() + "子だァ");
    }
}
