...
public class ikikaeru : MonoBehaviour
{
    ...
    {
        if (butukaru.gameObject.tag == "yuka") {
            t = targetObject[0];
            foreach (Transform spt in targetObject)
            {
                if (Vector3.Distance(transform.position, t.position) > Vector3.Distance(transform.position, spt.position))
                {
                    t = spt;
                }
            }
            while (n<targetObject.Length){
                if (t == targetObject[n])
                {
                    transform.rotation = Quaternion.Euler(0f, kaku[n], 0f);
                    break;
                }
                n++;
            }
